...
def adjust_box_to_origin(img, angle, result,imgname):
    """
    调整box到原图坐标
    """
    ...
    # 每个识别出来的文字的4角坐标x1,y1左上,x2,y2右上,x3,y3右下,x4,y4左下
    for line in result:
        cx = line['box']['cx']
        cy = line['box']['cy']
        ...
        # 颜色
        r_value=int(img_array[int((y1+y3)/2),int((x1+x3)/2),0])
        g_value=int(img_array[int((y1+y3)/2),int((x1+x3)/2),1])
        b_value=int(img_array[int((y1+y3)/2),int((x1+x3)/2),2])
        color=r_value,g_value,b_value
        fontsize=int(max(y1-y4,(x2-x1)/len(line['text'])))
        left_top=x1,y1
        right_bottom=x3,y3
        newresult.append({'no': line['name'], 'content': line['text'], 'left_top':left_top,'right_bottom':right_bottom,'fontsize':fontsize,'color':color})
